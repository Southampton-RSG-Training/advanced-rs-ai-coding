...
tools: [execute, read, edit/createFile, edit/editFiles, search, ms-python.python/getPythonEnvironmentInfo, ms-python.python/getPythonExecutableCommand, ms-python.python/installPythonPackage, ms-python.python/configurePythonEnvironment]
---
...
- Follow the architect's technical specification held in `project-docs/technical_spec.md` precisely
...
- Write code comments to explain reasoning when necessary, but avoid redundant comments
- Ensure code is modular and reusable where appropriate
- Avoid hardcoding values unless specified in the design
...
- Use the reST style for docstrings
...
- Use the `pytest` framework for test discovery and execution
- Keep the unit tests simple with minimal complexity
